...
> Find vulnerabilities, trace reachability, and audit open-source risk in your **Dart** project.

## About

This repository is a [Bomly](https://bomly.dev) example project for **Dart** with **pub**.
...
| `pubspec.lock` | Dependency manifest |
...
```bash
bomly scan --url https://github.com/bomly-dev/example-dart-pub --ecosystems dart
```
...
```bash
bomly scan --url https://github.com/bomly-dev/example-dart-pub --ecosystems dart --enrich --audit
```

### 3. Trace why a dependency exists

Understand every path that pulls `archive` into your graph:

```bash
bomly explain archive --url https://github.com/bomly-dev/example-dart-pub
```
